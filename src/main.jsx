...
function Header(){const [open,setOpen]=useState(false);const [expanded,setExpanded]=useState(null);const toggleGroup=name=>setExpanded(current=>current===name?null:name);const toggleMenu=()=>{setOpen(value=>{if(value)setExpanded(null);return !value})};useEffect(()=>{const close=e=>{if(e.key==='Escape'){setOpen(false);setExpanded(null)}};addEventListener('keydown',close);return()=>removeEventListener('keydown',close)},[]);return <>
  ...
  <header><div className="wrap headerin"><a className="brand" href="/"><img src="/images/logo.PNG" alt="People Action Development Society"/></a>
...
function HomeBody(){return <>
  ...
  <section className="giving section"><div className="wrap narrow center"><p className="eyebrow light">Community-led change</p><h2>Small actions create <em>lasting impact.</em></h2><p>Your support helps local teams turn practical ideas into sustainable education, health, water and livelihood initiatives.</p><a className="btn orange" href="/donating-methods/">Ways to support <ArrowRight size={18}/></a></div></section>
